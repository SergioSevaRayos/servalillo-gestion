    ...
    @else
        ...
        @if ($this->operable() && ! $this->finished && $this->pendingCount > 1)
            <button type="button" wire:click="optimizeFromBase" wire:target="optimizeFromBase"
                ...
                class="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 py-2.5 text-sm font-medium text-slate-500 transition-colors hover:border-primary-400 hover:text-primary-600 disabled:opacity-50 dark:border-slate-700 dark:text-slate-400 dark:hover:border-primary-500 dark:hover:text-primary-300">
                                ...
                                <div>
                                    @switch($field['type'])
                                        ...
                                        @case('textarea')
                                            ...
                                            @break
                                        @case('date')
                                            <x-ui.date-input :name="'fd_'.$field['key']" :label="$field['label']" wire:model="form.data.{{ $field['key'] }}" />
                                            @break
                                        @case('number')
                    ...
                    @else
                        ...
                        <div>
                            <x-ui.date-input name="form.reschedule_on" wire:model="form.reschedule_on"
                                ...
                                label="{{ __('Reprogramar para otro día (opcional)') }}"
                                help="{{ __('Se crea una parada nueva para ese día: en tu ruta si tienes una, o en \'Sin asignar\' para que oficina la reparta.') }}" />
                        </div>
                    @endif
